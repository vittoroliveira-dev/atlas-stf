...
def _load_process_dates(process_path: Path) -> dict[str, dict[str, Any]]:
    """Load process_id -> {filing_date, process_class, thematic_key}."""
...
def build_decision_velocity(
    *,
    ...
    for event in events:
        ...
        if not ctx:
            continue
        filing_date = _parse_date(ctx["filing_date"])
        decision_date = _parse_date(event.get("decision_date"))
        if not filing_date or not decision_date:
            continue
        days = (decision_date - filing_date).days
        if days < 0:
            continue
        decision_year = event.get("decision_year")
        if not decision_year:
            continue
        event_velocities.append(
            {
