...
def enrich_partner_record(record: dict[str, Any], qualificacoes: dict[str, str]) -> dict[str, Any]:
    ...
    record["qualification_label"] = qualificacoes.get(record.get("qualification_code", ""), "")
    record["representative_qualification_label"] = qualificacoes.get(record.get("representative_qualification", ""), "")
    return record
...
def enrich_company_record(record: dict[str, Any], naturezas: dict[str, str]) -> dict[str, Any]:
    ...
    record["natureza_juridica_label"] = naturezas.get(record.get("natureza_juridica", ""), "")
    return record
...
def enrich_establishment_record(
    ...
    record["motivo_situacao_label"] = motivos.get(record.get("motivo_situacao_cadastral", ""), "")
    # Decode secondary CNAEs
    sec = record.get("cnae_fiscal_secundaria", [])
    record["cnae_secundaria_labels"] = [cnaes.get(c.strip(), c.strip()) for c in sec if c.strip()]
    return record
