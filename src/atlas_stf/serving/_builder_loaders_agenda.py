    ...
    for r in _read_jsonl(path):
        ...
        if isinstance(ts, str) and ts:
            try:
                ...
                pass
        results.append(ServingAgendaEvent(
            event_id=eid, minister_slug=str(r.get("minister_slug", "")),
            minister_name=str(r.get("minister_name", "")),
            owner_scope=str(r.get("owner_scope", "")), owner_role=str(r.get("owner_role", "")),
            event_date=_parse_date(r.get("event_date")),
            event_time_local=tv, event_datetime_start=_parse_datetime(r.get("event_datetime_start")),
            source_time_raw=r.get("source_time_raw"), event_title=str(r.get("event_title", "")),
            ...
            participants_json=json.dumps({"participants_raw": r.get("participants_raw", []),
                "participant_entities": r.get("participant_entities", [])}, ensure_ascii=False),
            ...
            process_id=r.get("process_id"), process_class=r.get("process_class"),
...
def load_agenda_coverage(curated_dir: Path) -> list[ServingAgendaCoverage]:
    ...
    for r in _read_jsonl(path):
        ...
        if cid in seen:
            continue
        seen.add(cid)
        results.append(ServingAgendaCoverage(
            coverage_id=cid, minister_slug=str(r.get("minister_slug", "")),
            minister_name=str(r.get("minister_name", "")),
            owner_scope=str(r.get("owner_scope", "ministerial")),
            year=_coerce_int(r.get("year")), month=_coerce_int(r.get("month")),
...
def load_agenda_exposures(analytics_dir: Path) -> list[ServingAgendaExposure]:
    ...
    for r in _read_jsonl(path):
        ...
        if eid in seen:
            continue
        seen.add(eid)
        results.append(ServingAgendaExposure(
            exposure_id=eid, agenda_event_id=str(r.get("agenda_event_id", "")),
