...
function strengthLabel(score: number, uncertaintyNote: string | null): { text: string; tone: string } {
  ...
  if (score >= 0.7) {
    return { text: "Sinal moderado", tone: "text-amber-800 bg-amber-50" };
  }
  ...
  return (
    <section className="rounded-card border border-slate-200 bg-white p-6 shadow-elevation-1">
      <div className="max-w-3xl">
        <p className="text-xs font-semibold tracking-[0.02em] text-slate-500">Pontos de atenção</p>
      ...
      <div className="mt-6 grid gap-4">
        {meaningful.map((alert) => {
          ...
          return (
            <article key={alert.alert_id} className="rounded-inset border border-slate-200 bg-slate-50/70 p-4 transition hover:border-verde-300 hover:bg-verde-50/40">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="rounded-inset bg-white px-4 py-3 text-right shadow-sm">
                  <p className="text-xs font-semibold tracking-[0.02em] text-slate-500">Intensidade do sinal</p>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
                ...
                {alert.ensemble_score !== null ? (
                  <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
                    <p className="text-xs font-semibold tracking-[0.02em] text-slate-500">Score combinado</p>
