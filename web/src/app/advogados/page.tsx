...
export default async function CounselsPage({
  ...
  const data = await getEntityListPageData("counsel", {
    ...
    <AppShell
      ...
      heroState={
        data.filteredEntityCount === 0
            ? {
                status: "empty",
                title: "Nenhum representante neste período",
                description:
                  "Não encontramos representantes ligados ao filtro atual.",
            }
          : flow.thematic_flow_interpretation_status === "inconclusivo"
            ? {
                status: "inconclusivo",
                title: "Há representantes, mas ainda falta contexto para uma leitura mais firme",
                description:
                  "Os nomes aparecem no período, mas ainda exigem leitura cuidadosa do contexto.",
              }
            : {
                status: "ok",
                title: "Representantes ligados ao período selecionado",
                description:
                  "A lista ajuda a entender quem aparece com mais frequência e em que tipo de ligação.",
              }
      }
      eyebrow="Atlas STF · representantes"
      title="Representantes que aparecem neste período"
      description="Esta página mostra quais representantes aparecem no filtro atual e em quantos casos eles se repetem."
      guidance={{
        title: "Como usar esta lista",
        summary: "Use esta visão para encontrar nomes recorrentes e abrir o detalhe quando precisar entender os casos ligados a eles.",
        bullets: [
          "Veja primeiro quem aparece com mais frequência.",
          ...
          "Abra o detalhe para ver ministros relacionados e casos ligados a esse nome.",
        ],
      }}
    >
      <FilterBar
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <StatCard icon={Users} label="Representantes encontrados" value={String(data.filteredEntityCount)} help="Quantidade de representantes ligados ao filtro atual." />
      ...
      <EntityIndexGrid
        title="Representantes mais recorrentes"
        subtitle="A ordenação destaca quem aparece mais vezes no período e depois quem aparece em mais casos diferentes."
        items={data.entities}
        detailBasePath="/advogados"
        contextQuery={contextQuery}
        emptyMessage="Nenhum representante apareceu com estes filtros."
      />
